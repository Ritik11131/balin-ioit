...
import { Component, Input, Output, EventEmitter, OnInit, OnChanges, SimpleChanges, computed } from '@angular/core';
...
export interface FormField {
  key: string;
  label: string;
  type: 'text' | 'email' | 'password' | 'number' | 'textarea' | 'select' | 'multiselect' | 'checkbox' | 'radio' | 'date' | 'calendar' | 'autocomplete' | 'fileupload' | 'colorpicker';
  required?: boolean;
  placeholder?: string;
  value?: any;
    ...
    template: `
        <div class="w-full">
                            ...
                            @switch (field.type) {
                                @case ('text') {
                                    ...
                                    <ng-template #textInput>
                                        <input pInputText [id]="field.key" [formControlName]="field.key" [placeholder]="field.placeholder || ''" type="text" class="w-full" />
                                    </ng-template>
                                }
                                ...
                                @case ('email') {
                                    ...
                                    <ng-template #emailInput>
                                        <input pInputText [id]="field.key" [formControlName]="field.key" [placeholder]="field.placeholder || ''" type="email" class="w-full" />
                                    </ng-template>
                                }
                                ...
                                @case ('password') {
                                    ...
                                    <ng-template #commonInput>
                                        <input pInputText [id]="field.key" [formControlName]="field.key" [placeholder]="field.placeholder || ''" [type]="field.type" class="w-full" />
                                    </ng-template>
                                }
                                ...
                                @case ('textarea') {
                                    ...
                                    <ng-template #textareaInput>
                                        <textarea pInputTextarea [id]="field.key" [formControlName]="field.key" [rows]="field.rows || 3" class="w-full"></textarea>
                                    </ng-template>
                                }
                                ...
                                @case ('colorpicker') {
                                    <ng-container *ngTemplateOutlet="fieldWrapper; context: { field: field, input: colorPicker }"></ng-container>
            ...
            <ng-template #fieldWrapper let-field="field" let-input="input">
                <label [for]="field.key" class="font-medium"> {{ field.label }} <span class="text-red-500" *ngIf="field.required">*</span> </label>
...
export class GenericFormGeneratorComponent implements OnInit, OnChanges {
    ...
    private buildForm() {
        ...
        this.config.fields.forEach((field) => {
            ...
            formControls[field.key] = [{ value: initialValue, disabled: field.disabled === true }, validators];
        });
    ...
    onAutoCompleteSearch(event: any, field: FormField) {
        ...
        if (field.options) {
            this.filteredSuggestions = field.options.filter((option) => option.label.toLowerCase().includes(event.query.toLowerCase()));
        }
    }
