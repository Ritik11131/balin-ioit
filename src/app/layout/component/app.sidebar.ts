            ...
            <div class="flex flex-col items-center gap-4 mt-3 w-full px-7">
                @for (item of sidebarItems$ | async; track $index) {
                    <ng-container>
                        ...
                        >
                            <i [ngClass]="item.icon" class="text-lg transition-transform duration-200 group-hover:scale-110"></i>

                            <span
                                class="ml-3 text-sm font-medium truncate transition-all duration-200"
                                [ngClass]="{
